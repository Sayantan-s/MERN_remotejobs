...
const Footer = () => {
    ...
    const FooterRoutes = [
        ['About', 'Press', 'Partners Relations', 'Blog'],
        ['Job', 'Job Referrals', 'Contact Us', 'Help'],
        ['Careers', 'Blog', 'Help and Support', 'Affiliate'],
                    ...
                    <Flex width="100%" justifyContent="space-between" ml={10}>
                        {FooterRoutes.map((routes, id) => (
                            <StackVertical key={id} gap={6}>
                                {routes.map((link) => (
                                    <Link to="/" key={link} color="text.4" fontSize="1.8rem">
                                        {link}
                                    </Link>
                                ))}
                            </StackVertical>
